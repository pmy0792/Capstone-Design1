...
# from prompt_new import Prompt
# from prompt_new2 import Prompt
# from prompt_sub_P_head import Prompt
from prompt import Prompt
# from prompt_sub_P_head_V2 import Prompt
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            prompt_length=None, embedding_key='cls', prompt_init='uniform', prompt_pool=False, prompt_key=False, pool_size=None,pruning=False,pruning_option=None,
            task_prompt_size=None, top_k=None, batchwise_prompt=False, prompt_key_init='uniform', head_type='token', use_prompt_mask=False,
            num_tasks=10,epochs=5):
        """
        Args:
        ...
        if prompt_length is not None and pool_size is not None and prompt_pool:
            # embed_len += prompt_length * (pool_size+task_prompt_size)
            embed_len += prompt_length * (top_k+task_prompt_size)
            #! for the without clstoken
            embed_len -= 1  # ! remove position for the cls token
        ...
        self.train_prompt_selection=[[] for _ in range(num_tasks)]
        self.test_prompt_selection=[[] for _ in range(num_tasks)]
        
        # initialize for counting
        if pool_size is not None:            
            for idx,t in enumerate(self.train_prompt_selection):
                self.train_prompt_selection[idx]= [0 for _ in range(pool_size)]
                self.test_prompt_selection[idx]= [0 for _ in range(pool_size)]
                
        if prompt_length is not None and pool_size is not None and prompt_pool:
            self.prompt = Prompt(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key, prompt_init=prompt_init,
                                 prompt_pool=prompt_pool, prompt_key=prompt_key, pool_size=pool_size, task_prompt_size=task_prompt_size,pruning=pruning,
                                 num_classes=num_classes, top_k=top_k, batchwise_prompt=batchwise_prompt,
    ...
    def before_train(self, task_id):
        self.prompt.update_task_prompt_pool(task_id)
        self.prompt.update_assist_prompt(task_id)

    def forward_features(self, x, task_id=-1, cls_features=None, test=-1, threshold=0,epoch=-1): #test: test시 test하는 task number
        ...
        if hasattr(self, 'prompt'):
            if self.use_prompt_mask:
                ...
                prompt_mask = single_prompt_mask.unsqueeze(
            ...
            res = self.prompt(x, prompt_mask=prompt_mask,
                              cls_features=cls_features, test=test, task_id=task_id, threshold=threshold)
            self.total_prompt_len = res['total_prompt_len']
            x = res['prompted_embedding']
        else:
            res = dict()
        #! No use cls token
        if self.cls_token is not None and not hasattr(self, 'prompt'):
            x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)
        
        if hasattr(self,'prompt'):
            cloned_pos_embed = self.pos_embed.clone().detach() #[1,222,768]
            mask = torch.ones(cloned_pos_embed.shape, requires_grad=False, device="cuda") 
            
            iw_pruned_prompts = res['iw_pruned_prompts'] #[16, 5] pruned된거 -1으로들어감
            for idx,i in enumerate(list(iw_pruned_prompts)): # i : [ 4,6,8, -1,-1]
                
                # -1에 해당하는 부분을 mask에서 0으로 만들면됨!
                mask_start = -1
                mask_end = -1
                for _idx,p in enumerate(i):
                    # for counting prompt selection
                    if p>=0:
                        if test==-1: #when training
                            #print("epoch={}, self.epochs={}".format(epoch,self.epochs))
                        
                            if epoch==self.epochs-1:
                                
                                self.train_prompt_selection[task_id][p]+=1
                        else: # when testing
                            if (task_id==test):
                                self.test_prompt_selection[test][p]+=1
                            
                    if p<0: # location of pruned prompt
                        if mask_start==-1:
                            mask_start=_idx
                            mask_end=_idx
                        else:
                ...
                cloned_pos_embed *= mask
                x[idx] = self.pos_drop(x[idx] + cloned_pos_embed)    
        ...
        x = self.norm(x)
        res['x'] = x
        res['train_prompt_selection'] = self.train_prompt_selection
        res['test_prompt_selection'] = self.test_prompt_selection
    ...
    def forward(self, x, task_id=-1, cls_features=None, test=False, threshold=0,epoch=-1):
        res = self.forward_features(
            x, task_id=task_id, cls_features=cls_features, test=test,threshold=threshold,epoch=epoch )
...
def vit_large_patch14_224(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=14, embed_dim=1024,
                        depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer(
...
def vit_giant_patch14_224(pretrained=False, **kwargs):
    ...
    model_kwargs = dict(patch_size=14, embed_dim=1408,
                        mlp_ratio=48/11, depth=40, num_heads=16, **kwargs)
    model = _create_vision_transformer(
...
def vit_gigantic_patch14_224(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=14, embed_dim=1664,
                        mlp_ratio=64/13, depth=48, num_heads=16, **kwargs)
    model = _create_vision_transformer(
...
def vit_base_patch32_plus_256(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/32+)
    """
    model_kwargs = dict(patch_size=32, embed_dim=896, depth=12,
...
def vit_base_patch16_plus_240(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16+)
    """
    model_kwargs = dict(patch_size=16, embed_dim=896, depth=12,
                        num_heads=14, init_values=1e-5, **kwargs)
    model = _create_vision_transformer(
...
def vit_small_patch16_36x1_224(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=36,
